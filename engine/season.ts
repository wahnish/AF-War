...
import { ZONES, Zone, zoneById, bfsCorruptionOrder, corruptionSchedule, CORRUPTION_SOURCE } from './map.js'
...
export interface SeasonState {
    seed: string
    round: number
    zones: Map<string, { controlledBy?: string; corrupted: boolean; itemOnGround?: string }>
...
export function setupSeason(cfg: SeasonConfig): SeasonState {
    ...
    const zones = new Map<string, { controlledBy?: string; corrupted: boolean; itemOnGround?: string }>()
    for (const z of ZONES) zones.set(z.id, { corrupted: false })
    ...
    const candidates = shuffle(rng, ZONES.filter(z => !z.finale && z.id !== CORRUPTION_SOURCE && z.id !== 'monorail').map(z => z.id))
    ...
    const order = bfsCorruptionOrder(ring => shuffle(rng, ring))
...
export function attackableZones(s: SeasonState, crewId: string): string[] {
    ...
    return [...reach].filter(zid => {
        ...
        return !z.corrupted && z.controlledBy !== crewId && !zoneById(zid).finale
    })
}
...
export function playConvergence(s: SeasonState): RoundReport {
    ...
    while (bracket.length > 1) {
        ...
        for (let i = 0; i + 1 < bracket.length; i += 2) {
            ...
            const result = runMatch(rng, 'dodgers', 'death', bracket[i], bracket[i + 1])
            report.matches.push(result)
            if (result.consequence?.kind === 'death') {
                const dead = s.pcs.get(result.consequence.who)!
                dead.status = 'dead'
                s.pcs.get(result.consequence.killedBy)!.kills++
                events.push({ round: s.round, kind: 'death', pcId: dead.id, killedBy: result.consequence.killedBy, zoneId: 'dodgers' })
            }
    ...
    s.finished = true
    events.push({ round: s.round, kind: 'convergence', zoneId: 'dodgers', winnerCrew: champion.crewId })
    for (const crew of s.crews.values()) {
